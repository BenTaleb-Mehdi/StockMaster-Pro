<div x-data="{ open: false }" class="p-2 sm:p-4 lg:p-6">
    <div class="mb-5">
        ...
        <p class="text-sm text-gray-600">Manage your suppliers.</p>
    </div>
                ...
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-6 py-3 text-start text-xs font-semibold uppercase text-gray-500">Supplier</th>
                                <th class="px-6 py-3 text-start text-xs font-semibold uppercase text-gray-500">Phone</th>
                                <th class="px-6 py-3 text-start text-xs font-semibold uppercase text-gray-500">Address</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                          @foreach ($suppliers as $supplier)
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $supplier->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $supplier->phone }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $supplier->address }}</td>
                                <td class=" flex gap-x-3 justify-end px-8 py-4 whitespace-nowrap text-end text-sm font-medium">
                                    <button class="text-blue-600 hover:text-blue-800" wire:click="edit({{ $supplier->id}})" @click= "open = true"><i data-lucide="pencil" class="w-4 h-4 text-blue-600"></i>  </button>
                                    <button class="text-blue-600 hover:text-blue-800" wire:click="destory({{ $supplier->id}})"  wire:confirm="Are you sure you want to delete this product?"><i data-lucide="trash" class="w-4 h-4 text-red-600"></i></button>
                                </td>
                            </tr>
                            @endforeach
